...
class Rdict:
    ...
    def _recognize_key(self, key, struct_from):
        ...
        :rtype: object
        """
        if any([isinstance(struct_from, list), isinstance(key, bool)]):
            return key
    ...
    def get(self, path, default=None, raise_error=False):
        ...
        :rtype: object
        """
        indexes = path and path_to_array_keys(path)
        if not indexes:
            return self.copy()
        return deepcopy(self._recursive_get(indexes, self._dict, default, raise_error))
    ...
    def _recursive_search(self, value, dict_, current_path, is_key):
        ...
        for key_, value_ in it:
            ...
            deep_found = self._recursive_search(value, value_, new_path, is_key)
    ...
    def _recursive_search_pair(self, key, value, dict_, current_path):
        ...
        for key_, value_ in it:
            ...
            deep_found = self._recursive_search_pair(key, value, value_, new_path)
            found.extend(deep_found)
    ...
    def _recursive_del(self, indexes, dict_):
        ...
        dict_[current_index] = self._recursive_del(indexes[1:], dict_[current_index])
